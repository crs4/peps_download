...
def get_username_pwd(auth_file_path):
    if auth_file_path == '-':
        print("Reading authentication information from stdin")
        txt = sys.stdin.readline()
    else:
        with open(auth_file_path) as f:
            txt = f.readline()

    (email, passwd) = txt.split()
    passwd = passwd.trim('\n')
    return email, passwd
...
def parse_catalog(search_json_file, options):
...
def main(args):
    if len(args) == 0:
        ...
        print("        ou : ", prog, " -h")
        print("example 1 : python %s -l 'Toulouse' -a peps.txt -d 2016-12-06 -f 2017-02-01 -c S2ST" % prog)
        print("example 2 : python %s --lon 1 --lat 44 -a peps.txt -d 2015-11-01 -f 2015-12-01 -c S2" % prog)
        ...
        print("example 4 : python %s -l 'Toulouse' -a peps.txt -c SpotWorldHeritage -p SPOT4 -d 2005-11-01 -f 2006-12-01" %
              prog)
        print("example 5 : python %s -c S1 -p GRD -l 'Toulouse' -a peps.txt -d 2015-11-01 -f 2015-12-01" % prog)
        sys.exit(-1)
    else:
        ...
        parser.add_option("-a", "--auth", dest="auth", action="store", type="string",
                          help="Peps account and password file. Provide '-' to read 'USER PASS\\n' from standard input.")
    ...
    # ====================
    try:
        email, passwd = get_username_pwd(options.auth)
    except Exception:
    ...
    prod, download_dict, storage_dict = parse_catalog(options.search_json_file, options)
